...
test('@claim:desktop-download shows the detected platform asset from the current release before a visitor clicks', async ({ page }) => {
  await page.route('https://api.github.com/repos/B-Divyesh/sf-scan-repair-local/releases/latest', route => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ tag_name: 'v0.1.4', assets: [{ name: 'Scan.Repair.Local_0.1.4_amd64.AppImage', browser_download_url: 'https://github.com/B-Divyesh/sf-scan-repair-local/releases/download/v0.1.4/Scan.Repair.Local_0.1.4_amd64.AppImage' }] }) }))
  await page.goto('/')
  const download = page.locator('#download-link')
  await expect(download).toHaveText('Download for Linux')
  await expect(download).toHaveAttribute('href', /Scan\.Repair\.Local_0\.1\.4_amd64\.AppImage$/)
  await expect(page.locator('#platform-note')).toContainText('v0.1.4 is ready')
})
...
test('@claim:offline-demo reloads the demo after the first visit without a network connection', async ({ page, context }) => {
  ...
  const serviceWorker = await page.evaluate(async () => {
    ...
    const cache = await caches.open('scan-repair-local-v8'); const entries = await cache.keys()
