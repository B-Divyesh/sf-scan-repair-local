...
- Version surfaces are aligned at `0.1.5` across npm, Cargo, Tauri, Vite fallback copy, and legal/404 footers.
...
Release target: `v0.1.5`. The release workflow is required to pass its post-publish source identity and three-platform checksum download gate. After it publishes, rerun:

```sh
node scripts/verify-release.mjs --repo=B-Divyesh/sf-scan-repair-local --tag=v0.1.5 --sha="$(git rev-parse v0.1.5^{})"
```
